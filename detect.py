...
class HumanDetector:
    ...
    def detect_human(self, video_frame: np.ndarray) -> bool:
        video_frame = letterbox(video_frame, stride=self.stride)[0]
        video_frame = video_frame.transpose((2, 0, 1))[::-1]
        video_frame = np.ascontiguousarray(video_frame)
        tensor_frame = torch.from_numpy(video_frame).to(self.device).float()
        tensor_frame /= 255.0
        if len(tensor_frame.shape) == 3:
